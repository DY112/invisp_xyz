...
class InvertibleConv1x1(nn.Module):
    ...
    def get_weight(self, input, reverse):
        ...
        if not self.LU_decomposed:
            ...
            l_mask = self.l_mask.to(input.device)
            eye = self.eye.to(input.device)
            lower = self.lower.to(input.device)
            upper = self.upper.to(input.device)
            sign_s = self.sign_s.to(input.device)
            log_s = self.log_s.to(input.device)
            p = self.p.to(input.device)

            lower = lower * l_mask + eye

            u = upper * l_mask.transpose(0, 1).contiguous()
            u += torch.diag(sign_s * torch.exp(log_s))

            dlogdet = torch.sum(log_s) * h * w
            ...
            if reverse:
                ...
                weight = torch.matmul(p, torch.matmul(lower, u))
